...
public class Exercises {
	...
	public int diff21(int n) {
		if (n <= 21) {
			int rawDataFirst = 21 - n;
			return Math.abs(rawDataFirst);
		}
		else if (n > 21) {
			int rawDataSecond = 21 - n;
			return Math.abs(rawDataSecond)*2;
		}
		else {
			return 0;
		}
	...
	public int teaParty(int tea, int candy) {
		...
		if ((tea < 5) || (candy < 5))
			return 0;
		else if (tea >= 5 && candy >= 5 && (tea >= doubleCandy ) || (candy >= doubleTea))
			return 2;
