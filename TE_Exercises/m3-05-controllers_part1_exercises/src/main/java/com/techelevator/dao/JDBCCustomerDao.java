...
public class JDBCCustomerDao implements CustomerDao {
    ...
    @Override
	public List<Customer> searchAndSortCustomers(String search, String sortBy) {
		List<Customer> matchingCustomers = new ArrayList<>();
		search = search.toUpperCase();

		String customerByFnSQL = "SELECT first_name, last_name, email, activebool " + "FROM customer "
				+ "WHERE last_name = ? OR first_name = ? " + "ORDER BY ?";

		SqlRowSet results = jdbcTemplate.queryForRowSet(customerByFnSQL, search, search, sortBy);
		while (results.next()) {
