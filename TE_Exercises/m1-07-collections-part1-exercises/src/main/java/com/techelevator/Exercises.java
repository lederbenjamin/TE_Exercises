...
public class Exercises {
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (int i = 0; i < integerArray.length; i++) {
			if ((integerArray[i] % 5 == 0) && (integerArray[i] % 3 == 0)) {
				...
				holder = Integer.toString(integerArray[i]);
				integerList.add(holder);
			}
		}
		return integerList;
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		List<Integer> together = new ArrayList<Integer>();
		...
		for (int i = 0; i < seatNumberList.size(); i++) {
			if ((seatNumberList.get(i) > 0) && (seatNumberList.get(i) <= 30)) {
				if ((seatNumberList.get(i) > 0) && (seatNumberList.get(i) <= 10))
					group1.add(seatNumberList.get(i));
				else if ((seatNumberList.get(i) > 10) && (seatNumberList.get(i) <= 20))
					group2.add(seatNumberList.get(i));
				else if ((seatNumberList.get(i) > 20) && (seatNumberList.get(i) <= 30))
					group3.add(seatNumberList.get(i));
			}
		}
		together.addAll(group1);
		together.addAll(group2);
		together.addAll(group3);
