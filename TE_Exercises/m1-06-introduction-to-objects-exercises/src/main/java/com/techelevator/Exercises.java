...
public class Exercises {
	...
	public String extraEnd(String str) {
		String original = str.substring(str.length() - 2);
	...
	public boolean doubleX(String str) {
		...
		int ofX = str.indexOf("x");
		if (ofX < 0)
			return status;
		else if (str.substring(ofX).startsWith("xx"))
			status = true;	
			return status;
	}
	...
	public String stringX(String str) {
		...
		if (str.substring(0, 1).equals("x"))
			store += str.substring(0,1);
		for (int i = 0; i < str.length(); i++) {
			if (!str.substring(i, i+1).equals("x"))
				store += str.substring(i,i+1);
		}
		if (str.substring(str.length()-1).equals("x"))
			store += str.substring(str.length()-1);
		return store;
	}
	...
	public String altPairs(String str) {
		...
		for (int i = 0; i < str.length(); i+=4) {
			...
			if (finalString > str.length())
				finalString = str.length();
			
			store += str.substring(i,finalString);
		}
		return store;
}
